...
- 2026-06-16: Review PM17 并修复评测闭环一致性：`benchmark.py` 改为复用 `evaluation.statistics.bootstrap_ci/cohens_d`，保留 `_bootstrap_ci()` 私有 wrapper 兼容旧测试；`compare_summaries()` 纳入 `cohens_d_easy_vs_hard` delta；`RunManager` 默认向 Langfuse metadata 写入 `model_backend` 和 `prompt_label`，避免只有 benchmark 显式传参时才可见。
