...
- Milvus 作为主向量数据库。
- Milvus Lite 用于早期开发和 demo。
- 后续使用 Docker 部署 Milvus Standalone。
...
- Milvus Distributed：MVP 先用 Milvus Lite，避免一开始引入 K8s 和多节点运维。
...
- Python + asyncio + Milvus Lite + Typer。
- 单机执行，Milvus Lite 作为本地向量服务。
