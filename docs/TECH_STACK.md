...
MVP 必须支持本地资料集，因为可复现、成本低、测试稳定；同时要支持 Tavily 真实搜索、MiMo 原生搜索和轻量正文抓取。未实现的数据源扩展想法统一放在 `archive/IDEAS.md`。
...
- `MemoryStore` 接口保留测试替身和未来替换空间，但当前主线只维护 Milvus 和 mock。
...
- 额外向量数据库或分布式运维：暂不作为主线，避免检索层过早分叉。
