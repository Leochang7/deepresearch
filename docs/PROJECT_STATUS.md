...
- LexicalPolicy 支持 `builtin` 和 `jieba` tokenizer，内置项目术语 userdict，默认保留 CJK ngram fallback
...
- **检索质量**: Tavily 结果偏向英文；中文查询已通过本地跨语言资料集优化，但联网搜索质量仍受 provider 影响；builtin CJK unigram/bigram 保证离线稳定，jieba tokenizer 可通过配置启用
