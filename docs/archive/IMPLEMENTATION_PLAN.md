...
6. 使用 Qwen3-Embedding-4B 生成配置维度的 embedding。真实验收环境为 2560 维。
7. chunk、embedding 和元数据写入 Milvus。
8. Research Agent 从 Milvus 召回 top 30 证据片段。
9. 使用配置的 reranker rerank 到 top 8。真实验收环境为 `bge-reranker-v2-m3`。
...
- dim: 由 endpoint `/models` 和配置共同确定；真实验收环境为 `2560`
- batch size: `32`
- timeout: `60s`
- max retries: `2`
- normalize: `false`
...
- model: 由 endpoint `/models` 和配置共同确定；真实验收环境为 `bge-reranker-v2-m3`
- batch size: `16`
- timeout: `60s`
- max retries: `2`
