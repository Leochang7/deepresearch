...
### Phase 7：Langfuse 评测闭环

- 接入 Langfuse，用于 trace、dataset、experiment run、score 记录和对比展示。
- 本项目保留 ResearchBench mini、benchmark runner、规则指标和可复现实验入口。
- 规则指标扩展到事实覆盖、引用支持度、幻觉风险。
- LLM-as-Judge 五维评分作为本地 evaluator 能力，并可上报 Langfuse scores。
- 完整 ResearchBench 11 领域/35 题、HotpotQA 深度研究变体、Bootstrap 95% 置信区间、Cohen's d 和多后端实验脚本后置到 pipeline 稳定之后。

### Phase 8：真实 benchmark 质量校准

- 将 `factual_hit_rate` 从整句 token overlap 升级为 fact-level 规则 + semantic judge。
- 每条 expected fact 输出 `hit/miss/uncertain`、supporting evidence ids 和 reason。
- `results.jsonl` 与 `summary.json` 输出 per-fact、per-case failure reason。
- 先跑真实 5-case smoke，确认指标能解释失败，再跑完整 ResearchBench mini。
- 将真实结果和复现实验命令同步到 `docs/REAL_BENCHMARK_GUIDE.md` 和 `docs/PROJECT_STATUS.md`。
...
- 增加报告版本对比。
- 增加 Red-Blue 修复 diff。
- benchmark dashboard 优先使用 Langfuse，不在本项目初期自研。
