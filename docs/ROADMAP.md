...
- 验证：默认离线测试通过，最新记录为 `603 passed, 1 skipped`，ruff clean。
...
### PM22-PM25 Langfuse Evaluation Ops

PM21 已完成评测 schema hardening。后续如果继续增强 Langfuse，不再新增一套本地评测平台，而是把现有本地 dataset、runner、metrics、judge 和 trace 更好地映射到 Langfuse 的评测协作能力。

- PM22 Langfuse Dataset & Experiment Binding：将 `examples/bench/*.jsonl` bootstrap 为 Langfuse dataset items，并让 benchmark trace 关联 `dataset_name + case_id`，用于 UI 内对比模型、prompt label、语言场景和分数。本地 JSONL 继续作为离线 source of truth。
- PM23 Fine-grained Langfuse DAG/Agent Observations：把 Planner、DAG task、retrieval query、memory search、synthesis、Red/Blue/Judge 和 Evaluator 记录为 nested observations，并标准化 RunBudget、latency、token usage 和检索数量。
- PM24 Langfuse-managed Evaluator & Prompt Versioning：记录 runtime prompt name/version/hash 到 trace metadata；支持 LLM-as-Judge 的 prompt/model/label 由 Langfuse 管理，本地 judge prompt 继续作为 fallback。
- PM25 Human Annotation Queue & Review Loop：将低分或争议 benchmark traces 推入人工标注队列，并把人工标注结果回流为本地 summary 附加层，用于校准规则指标和 LLM-as-Judge。
