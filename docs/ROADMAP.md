...
## 当前完成：PM16-PM25 完整评测与 Langfuse Ops

PM16-PM25 已经把 benchmark 能力扩展成可复现实验平台，并把本地评测闭环映射到 Langfuse。当前稳定评测主线是 local corpus，不依赖 UI 或联网搜索；Langfuse 作为 prompt、trace、score、experiment 和人工审阅协作层。
...
- 验证：默认离线测试通过，最新记录为 `644 passed, 1 skipped`，ruff clean。
