...
from deepresearch.evaluation.fact_matching import (
    FactSpec,
    _detect_language,
    _evaluate_fact,
    _fact_in_text,
...
def evaluate(
    ...
    expected_facts: list[FactSpec] | None = None,
    ...
    fact_details: list[FactHitResult] = []
    per_fact_failure_reasons: list[FactFailureReason] = []
    if expected_facts:
        ...
        fact_details = results
        per_fact_failure_reasons = [
            FactFailureReason(fact=r.fact, reason=r.reason)
            for r in results
            if not r.matched
        ]
